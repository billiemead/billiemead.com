								...
								if ( get_option("carbon_telephone_menu") != "" || get_option("carbon_email_menu") != "" || get_option("carbon_address_menu") != "" || get_option("carbon_text_field_menu") != "" ){
									...
									<ul class="phone-mail">
										...
										<?php if ( is_string(get_option("carbon_text_field_menu")) && get_option("carbon_text_field_menu") != "" ){ ?>
											<li><i class="fa fa-info-circle"></i><?php echo wp_kses_post(get_option("carbon_text_field_menu")); ?></li>
										<?php } ?>
									</ul>
									<?php
								}
		...
		<div class="nav-container <?php if (get_option("carbon_header_full_width") == "off") echo " container"; ?>">
			...
			<div class="navbar-collapse collapse">
				...
				<?php 
					...
					<?php
					//search trigger
					if (get_option("carbon_enable_search") == "on" && $typeofheader != "style4"){
						?>
						<div class="search_trigger"><i class="ion-ios-search-strong"></i></div>
						<?php
					}
					
					carbon_print_woocommerce_button();
			...
			<?php
				if (!$isfirstpage){
					...
					<div id="dl-menu" class="dl-menuwrapper">
						...
						<?php 
							if ($ispagephp){
								wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'dl-menu', 'walker' => new carbon_walker_nav_menu_outsider, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','carbon') ) );
							} 
							else {
								global $homes;
								$homes = 0;
								wp_nav_menu( array( 'theme_location' => 'PrimaryNavigation', 'container' => false, 'menu_class' => 'dl-menu', 'walker' => new carbon_walker_nav_menu, 'fallback_cb' => esc_html__('You need to assign a Menu to the Main Navigation Location.','carbon') ) );
							} 
						?>
					</div>
					<?php
			...
			?>
				<form autocomplete="off" role="search" method="get" class="search_input <?php echo esc_attr(get_option("carbon_search_open_effect")); ?>" action="<?php echo home_url( '/' ); ?>">
					<?php
			...
			<div class="header_style2_contact_info">
				<?php
					if (get_option('carbon_style4_email_menu') != ""){
						?>
						<div class="email-contact">
							<div class="icon"><i class="ion-at"></i></div>
							<div class="details">
								<?php
									if (get_option('carbon_style4_email_slogan') != ""){
										?>
										<div class="slogan"><?php echo esc_html(get_option('carbon_style4_email_slogan')); ?></div>
										<?php
									}
								?>
								<div class="email"><?php echo esc_html(get_option('carbon_style4_email_menu')); ?></div>
							</div>
						</div>
						<?php
					}
					if (get_option('carbon_style4_telephone_menu') != ""){
						?>
						<div class="telephone-contact">
							<div class="icon"><i class="ion-ios-telephone"></i></div>
							<div class="details">
								<?php
									if (get_option('carbon_style4_telephone_slogan') != ""){
										?>
										<div class="slogan"><?php echo esc_html(get_option('carbon_style4_telephone_slogan')); ?></div>
										<?php
									}
								?>
								<div class="email"><?php echo esc_html(get_option('carbon_style4_telephone_menu')); ?></div>
							</div>
						</div>
						<?php
					}
