<?php
	...
	if ($type != "without"){
		...
		?>
		<div class="fullwidth-container <?php if ($type == "pattern") echo "bg-pattern"; ?> <?php if ($pt_parallax) echo "parallax"; ?><?php if (($type == "image" || $type == "pattern" || $type == "featured_image") && get_option('carbon_enable_grayscale') == 'on') echo " carbon_grayscale "; ?>" <?php if ($pt_parallax) echo 'data-stellar-ratio="0.5"'; ?>
	    	<?php
		    	$carbon_output = ".fullwidth-container{";
		    	if ($type == "featured_image"){
			    	$type = "image";
			    	$image = wp_get_attachment_url( get_post_thumbnail_id($carbon_thisPostID) );
				}
				...
				<div class="container present-container <?php echo esc_attr($originalalign); ?>">
					...
					<?php
						...
						if ($showtitle){
							?>
							<h1 class="page_title">
								<?php echo wp_kses_post(get_the_title($carbon_thisPostID)); ?>
							</h1>
							<?php
		    			...
		    			if ($showsectitle){
			    			if (is_string(get_post_meta($post->ID, 'secondaryTitle_value', true)) && get_post_meta($post->ID, 'secondaryTitle_value', true) != ""){
								...
								if (is_string(get_option('carbon_blog_secondary_title')) && get_option('carbon_blog_secondary_title') != ""){
									?>
								    <h2 class="secondaryTitle">
								    	<?php echo wp_kses_post(get_option('carbon_blog_secondary_title')); ?>
								    </h2>
									<?php
								}
							}
					...
					if ((get_post_meta( $carbon_thisPostID , 'upper_single_display_metas_value' , true ) == "yes" || get_post_meta( $carbon_thisPostID , 'upper_single_display_metas_value' , true ) == "") && (get_post_meta( $carbon_thisPostID , 'upper_single_display_metas_where_value' , true ) == 'pagetitle' || get_post_meta( $carbon_thisPostID , 'upper_single_display_metas_where_value' , true ) == '')){
						...
						<div class="metas-container">
							<div class="align-metas-center">
				    			...
				    			<?php
					    			$metas = get_post_meta( $carbon_thisPostID , 'upper_single_metas_value' , true );
					    			if ($metas=="") $metas = "date,author,tags,categories";
					    			$metas = explode(",", $metas);
					    			if (!empty($metas)){
						    			$carbon_output = ".carbon-apply-tcolor{ color:#".esc_attr($tcolor)." !important; }";
						    			carbon_set_custom_inline_css($carbon_output);
						    			$firstMeta = true;
						    			foreach ($metas as $meta){
							    			switch ($meta){
								    			case "date":
									    			if (!$firstMeta){
										    			echo '<p class="metas-sep carbon-apply-tcolor">|</p>';
									    			} else {
										    			$firstMeta = false;
									    			} 
								    				?>
								    				<p class="blog-date carbon-apply-tcolor"><?php echo get_the_date(); ?></p>
								    				<?php
								    			break;
								    			case "author":
								    				if (!$firstMeta){
								    				...
								    				?>
								    				<p class="carbon-apply-tcolor"><?php
								    				...
								    				?>: <a class="the_author carbon-apply-tcolor" href="<?php echo get_author_posts_url( get_the_author_meta('ID') ); ?>"> <?php  esc_html(the_author_meta('nickname')); ?></a></p>
								    				<?php
								    			break;
								    			case "tags":
													...
													if ($posttags) {
														...
														$first = true;
														echo '<p class="carbon-apply-tcolor">';
														if (function_exists('icl_t')){
														...
														foreach($posttags as $tag) {
															if ($tag->name != "uncategorized"){
																if ($first){
																	echo "<a class='carbon-apply-tcolor' href='".esc_url( get_term_link($tag) )."'>".esc_html($tag->name)."</a>"; 
																	$first = false;
																} else {
																	echo "<span>, </span><a class='carbon-apply-tcolor' href='".esc_url( get_term_link($tag) )."'>".esc_html($tag->name)."</a>";
																}
															}
													  	}
													  	echo '</p>';
													...
													if ($postcats) {
														...
														foreach($postcats as $cat) {
															if ($cat->name != "uncategorized"){
																if ($first){
																	echo "<a class='carbon-apply-tcolor' href='".esc_url( get_term_link($cat) )."'>".esc_html($cat->name)."</a>"; 
																	$first = false;
																} else {
																	echo "<span>, </span><a class='carbon-apply-tcolor' href='".esc_url( get_term_link($cat) )."'>".esc_html($cat->name)."</a>"; 
																}	
															}
													  	}
													  	echo '</p>';
								    			...
								    			case "customtext":
								    				if (!$firstMeta){
										    			echo '<p class="metas-sep" class="carbon-apply-tcolor">|</p>';
									    			} else {
										    			$firstMeta = false;
									    			}
								    				echo '<p class="carbon-apply-tcolor">'.wp_kses_post( get_post_meta( $carbon_thisPostID , 'upper_single_meta_custom_text_value' , true ) ).'</p>';
								    			break;
							    			}
						    			}
					    			}
	...
	$is_custom_sidebar_layout = get_post_meta($carbon_thisPostID, 'enable_post_custom_sidebar_layout_value', true) == "true" ? true : false;
	$sidebar_scheme = $is_custom_sidebar_layout ? get_post_meta( $carbon_thisPostID, 'sidebar_for_default_value', true ) : get_option('carbon_blog_single_sidebar');
	$sidebar = $is_custom_sidebar_layout ? carbon_convert_to_class(get_post_meta( $carbon_thisPostID, 'sidebars_available_value', true )) : carbon_convert_to_class(get_option('carbon_sidebars_available'));
	...
	function carbon_print_single_post(){
		...
		<article id="post-<?php esc_attr(the_ID()); ?>" class="post">
	    	...
	    	<?php
	    		...
	    		switch($posttype){
		    		case "image":
		    			if (wp_get_attachment_url( get_post_thumbnail_id($postid)) && get_post_meta( $postid, 'carbon_display_featured_image_value', true ) == "yes" ){
						?>
		    		...
		    		case "slider": 
						...
						<?php
						$animation = get_option("carbon_posts_flex_transition");
						$directionNav = get_option("carbon_posts_flex_navigation");
						$slideshowSpeed = get_option("carbon_posts_flex_slide_duration") ? get_option("carbon_posts_flex_slide_duration") : 3000;
						$pauseOnHover = get_option("carbon_posts_flex_pause_hover");
						$controlNav = get_option("carbon_posts_flex_controls");
						$slideshow = get_option("carbon_posts_flex_autoplay");
						$height = get_option("carbon_posts_flex_height");
						$animationDuration = get_option("carbon_posts_flex_transition_duration") ? get_option("carbon_posts_flex_transition_duration") : 1000;
	    		...
	    		<div class="the_content">
			    	<?php 
			    		the_content();
			    		$args = array(
							'before'           => '<div class="navigation margin-top-0px"><div class="des-pages"><span class="pages current">' . esc_html__('Post Pages:', 'carbon') . '</span>',
							'after'            => '</div></div>',
							'link_before'      => '<div class="postpagelinks">',
							'link_after'       => '</div>',
							'next_or_number'   => 'number',
							'nextpagelink'     => esc_html__('Next page','carbon'),
							'previouspagelink' => esc_html__('Previous page','carbon'),
							'pagelink'         => '%',
							'echo'             => 1
						); 
			    		wp_link_pages($args); 
	    		...
	    		<?php
					if (get_option("carbon_post_single_social_shares") == "on" && get_option('carbon_post_single_socials') != "" && defined('CARBON_PLG_ACTIVE')){
					$post_single_socials = explode(",",get_option('carbon_post_single_socials'));
					?>
					<div class="share-buttons">
		                
			        	<h5><?php
				        	if (function_exists('icl_t')){
					        	echo sprintf(esc_html__("%s","carbon"), icl_t( 'carbon', 'SHARE THIS', get_option('carbon_share_post_text')));
				        	} else {
					        	echo sprintf(esc_html__("%s","carbon"), get_option("carbon_share_post_text"));
				        	}
			        	?></h5>
				...
				?>    
				 ...
				 <nav id="nav-below" role="article" class="navigation">
					<?php 
						if (function_exists('icl_t')){
							previous_post_link( '<div class="nav-previous"><i class="ion-ios-arrow-thin-left"></i>%link</div>', sprintf(esc_html__("%s",'carbon'), icl_t( 'carbon', 'Previous post', get_option('carbon_single_previous_text')))); 
							next_post_link( '<div class="nav-next">%link<i class="ion-ios-arrow-thin-right"></i></div>', sprintf(esc_html__("%s",'carbon'), icl_t( 'carbon', 'Next post', get_option('carbon_single_next_text')) )); 
						} else {
							previous_post_link( '<div class="nav-previous"><i class="ion-ios-arrow-thin-left"></i>%link</div>', sprintf(esc_html__("%s",'carbon'), get_option('carbon_single_previous_text'))); 
							next_post_link( '<div class="nav-next">%link<i class="ion-ios-arrow-thin-right"></i></div>', sprintf(esc_html__("%s",'carbon'), get_option('carbon_single_next_text') )); 
						}
					?>
				</nav>
				
				<?php
					if (get_option("carbon_post_about_author") == "on"){
						?>
