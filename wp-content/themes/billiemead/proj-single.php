<?php
	    	...
	    	<?php
				...
				<div class="container present-container <?php echo esc_attr($originalalign); ?>">
					...
					<?php
		    			...
		    			if ($showsectitle){
			    			if (get_post_meta($carbon_thisPostID, 'secondaryTitle_value', true) != "" || get_option('carbon_blog_secondary_title') != ""){
						    	?>
							    <h2 class="secondaryTitle">
							    	<?php echo wp_kses_post(get_post_meta($carbon_thisPostID, 'secondaryTitle_value', true)); ?>
							    </h2>
					    		<?php
	...
	$singleLayout = get_post_meta($carbon_thisPostID, 'singleLayout_value', true);
	if ($singleLayout == "default"){
		$singleLayout = get_option("carbon_single_layout");
	}

	if (get_post_meta($carbon_thisPostID, "portfolioType_value", true) != "other") {
		$pj_cols = " col-md-7";
		$ct_cols = " col-md-5";
		...
		<article id="post-<?php esc_attr(the_ID()); ?>" <?php post_class(); ?> role="article">
			...
			<?php
				if (post_password_required()) {
						...
						<div class="projects_description">
							<div class="projects_media <?php echo esc_attr($singleLayout . $pj_cols); ?>">
								<?php
									...
									if (get_post_meta($carbon_thisPostID, "portfolioType_value", true) == "image"){
										$output .= "<div id='p-slider-".esc_attr(get_the_ID())."' class='flexslider clearfix'><ul class='slides da-thumbs-plus'>";
										
										$sliderData = get_post_meta($carbon_thisPostID, "sliderImages_value", true);
									...
									if (get_post_meta($carbon_thisPostID, "portfolioType_value", true) == "video") {
										$videosType = get_post_meta($carbon_thisPostID, "videoSource_value", true);
										if ($videosType != "embed"){
											$videos = get_post_meta($carbon_thisPostID, "videoCode_value", true);
											$videos = preg_replace( '/\s+/', '', $videos );
											$vid = explode(",",$videos);
										}
										switch (get_post_meta($carbon_thisPostID, "videoSource_value", true)){
											case "media":
												$output .= "<video id='html5video' preload='metadata' controls='controls' >";
												$media = get_post_meta($carbon_thisPostID, 'videoMediaLibrary_value', true);
												$media = explode("|*|",$media);
												foreach ($media as $m){
													if (strlen($m) > 0){
											...
											case "vimeo":
												...
												break;
											case "embed":
												$output .= "<div class='embedded'>".get_post_meta($carbon_thisPostID, "videoCode_value", true)."</div>";
												break;
										}
									}
							...
							<div class="content_container <?php echo esc_attr($ct_cols); ?>">
								<?php 
									...
									carbon_content_shortcoder($content);
					...
					<?php
						if (get_option("carbon_project_single_social_shares") == "on" && get_option('carbon_project_single_socials') != ""){
						$proj_single_socials = explode(",",get_option('carbon_project_single_socials'));
						?>
						<div class="share-buttons">
			                
				        	<h5><?php 
					        	if (function_exists('icl_t')){
						        	echo sprintf(esc_html__("%s","carbon"), icl_t( 'carbon', 'SHARE THIS PROJECT', get_option('carbon_share_proj_text'))); 
					        	} else {
						        	echo sprintf(esc_html__("%s","carbon"), get_option("carbon_share_proj_text")); 
					        	}
					        ?></h5>
							        ...
							        <ul>
								        <?php
									        if (in_array("facebook", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php echo esc_url("https://www.facebook.com/sharer.php?u=".get_the_permalink()."&amp;t=".get_the_title()); ?>" class="share-facebook" target="_blank" ><i class="fa fa-facebook"></i><?php esc_html_e( '', 'carbon' )?></a>
												</li>
										        <?php
									        }
									        if (in_array("twitter", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php echo esc_url("https://twitter.com/intent/tweet?text=".get_the_title()."&url=".get_the_permalink()); ?>" class="share-twitter" target="_blank" ><i class="fa fa-twitter"></i><?php esc_html_e( '', 'carbon' )?></a>
												</li>
										        <?php
									        }
									        if (in_array("linkedin", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php echo esc_url("https://linkedin.com/shareArticle?mini=true&amp;url=".get_the_permalink()."&amp;title=".get_the_title()); ?>" target="_blank" class="share-linkedin" ><i class="fa fa-linkedin"></i><?php esc_html_e( '', 'carbon' )?></a>
												</li>
										        <?php
									        }
									        if (in_array("pinterest", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php $url = wp_get_attachment_url( get_post_thumbnail_id(get_the_ID()) ); echo esc_url("https://www.pinterest.com/pin/create/button/?url=".get_the_permalink()."&amp;media=".$url."&amp;") ?>" target="_blank" class="share-pinterest" ><i class="fa fa-pinterest"></i><?php esc_html_e( '', 'carbon' )?></a>
												</li>
										        <?php
									        }
									        if (in_array("tumblr", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php echo esc_url("https://www.tumblr.com/share/?url=".get_the_permalink()."&amp;title=".get_the_title()); ?>" target="_blank" ><i class="fa fa-tumblr"></i><?php esc_html_e( '', 'carbon' )?></a>							
												</li>
										        <?php
									        }
									        if (in_array("email", $proj_single_socials)){
										        ?>
										        <li>
													<a href="<?php echo esc_url("mailto:?subject=".get_the_title()."&amp;body=".get_the_permalink()); ?>" class="share-mail" ><i class="fa fa-envelope-o"></i> <?php esc_html_e( '', 'carbon' )?></a>
												</li>
										        <?php
									        }
									        
									        if (!isset($carbon_inline_script)) $carbon_inline_script = '';
			        ...
			        <div class="projects_nav1 container">
						<?php 
							if (function_exists('icl_t')){
								previous_post_link( '<div class="nav-previous-nav1">%link</div>', sprintf(esc_html__("%s","carbon"), icl_t( 'carbon', 'Previous Project', get_option('carbon_prev_single_proj')) )); 
								next_post_link( '<div class="nav-next-nav1">%link</div>', sprintf(esc_html__("%s", "carbon"), icl_t( 'carbon', 'Next Project', get_option('carbon_next_single_proj')) )); 
							} else {
								previous_post_link( '<div class="nav-previous-nav1">%link</div>', sprintf(esc_html__("%s","carbon"), get_option("carbon_prev_single_proj") )); 
								next_post_link( '<div class="nav-next-nav1">%link</div>', sprintf(esc_html__("%s", "carbon"), get_option("carbon_next_single_proj") )); 
							}
						?>
					</div>
		...
		//image
		if (get_post_meta($carbon_thisPostID, "portfolioType_value", true) == "image"){
			if (get_post_meta($carbon_thisPostID, "custom_slider_opts_value", true) == "on"){
				$animation = get_post_meta($carbon_thisPostID, "projs_flex_transition_value", true);
				$directionNav = get_post_meta($carbon_thisPostID, "projs_flex_navigation_value", true);
				$slideshowSpeed = get_post_meta($carbon_thisPostID, "projs_flex_slide_duration_value", true) != "" ? get_post_meta($carbon_thisPostID, "projs_flex_slide_duration_value", true) : 3000;
				$pauseOnHover = get_post_meta($carbon_thisPostID, "projs_flex_pause_hover_value", true);
				$controlNav = get_post_meta($carbon_thisPostID, "projs_flex_controls_value", true);
				$slideshow = get_post_meta($carbon_thisPostID, "projs_flex_autoplay_value", true);
				$height = get_post_meta($carbon_thisPostID, "projs_flex_height_value", true);
				$animationDuration = get_post_meta($carbon_thisPostID, "projs_flex_transition_duration_value", true) != "" ? get_post_meta($carbon_thisPostID, "projs_flex_transition_duration_value", true) : 1000;
			} else {
				$animation = get_option("carbon_projs_flex_transition");
				$directionNav = get_option("carbon_projs_flex_navigation");
				$slideshowSpeed = get_option("carbon_projs_flex_slide_duration") ? get_option("carbon_projs_flex_slide_duration") : 3000;
				$pauseOnHover = get_option("carbon_projs_flex_pause_hover");
				$controlNav = get_option("carbon_projs_flex_controls");
				$slideshow = get_option("carbon_projs_flex_autoplay");
				$height = get_option("carbon_projs_flex_height");
				$animationDuration = get_option("carbon_projs_flex_transition_duration") ? get_option("carbon_projs_flex_transition_duration") : 1000;
			}
			...
			$carbon_inline_script .= '
				if (jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").find("li").length > 1){
					jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").css("opacity",0).flexslider({
						animation: "'.esc_js($animation).'",
						slideDirection: "horizontal", 
						directionNav: "'.esc_js($directionNav).'",
						slideshowSpeed: '.esc_js($slideshowSpeed).',
						controlsContainer: "#p-slider-'.esc_js($carbon_thisPostID).' .flex-viewport",
						pauseOnAction: false,
						...
						start: function(slider){
							...
							jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").css("opacity",1);
					...
					jQuery("#p-slider-'.esc_js($carbon_thisPostID).' ul.slides").css({"max-height":"'.esc_js($height).'"});
				} else {
					jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").find("ul li").css("display","block");
					jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").find("li a img").css("opacity",1);
					jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").find(".magnifier").on("click", function(){
						jQuery("#p-slider-'.esc_js($carbon_thisPostID).'").find("li a").trigger("click");
					});
				}
			';
		}
		//video
		if (get_post_meta($carbon_thisPostID, "portfolioType_value", true) == "video") {
			if (get_post_meta($carbon_thisPostID, "videoSource_value", true) != "embed" && get_post_meta($carbon_thisPostID, "videoSource_value", true) != "media"){
				$carbon_inline_script .= '
		...
		$carbon_output = ".nav-previous-nav1 a, .nav-next-nav1 a{ color: rgb(102, 102, 102); opacity: 0.3; filter: alpha(opacity=30); }";
		carbon_set_custom_inline_css($carbon_output);
					        ...
					        <ul>
						        <?php
							        ...
							        if (in_array("twitter", $proj_single_socials)){
								        ?>
								        <li>
											<a href="<?php echo esc_url("https://twitter.com/home?status=".get_the_title()."_".get_the_permalink()); ?>" class="share-twitter" target="_blank" ><i class="fa fa-twitter"></i><?php esc_html_e( '', 'carbon' )?></a>
										</li>
								        <?php
							        }
							        if (in_array("linkedin", $proj_single_socials)){
							        ...
							        if (in_array("googleplus", $proj_single_socials)){
								        ?>
								        <li>
											<a href="<?php echo esc_url("https://google.com/bookmarks/mark?op=edit&amp;bkmk=".get_the_permalink()."&amp;title=".get_the_title()); ?>" target="_blank" class="share-google" ><i class="fa fa-google-plus"></i><?php esc_html_e( '', 'carbon' )?></a>
										</li>
								        <?php
							        }
							        if (in_array("pinterest", $proj_single_socials)){
