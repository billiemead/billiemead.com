var carbonOptions = {
    ...
    init: function (options) {
        carbonOptions.setCheckboxClickHandlers();
        carbonOptions.setHelpFunc();
        carbonOptions.setOnOffFunc();
        carbonOptions.setTextImageFunc();
        carbonOptions.setLeftRightFunc();
        carbonOptions.setLightDarkFunc();
        carbonOptions.setColorpickFunc();
        carbonOptions.setStyleSelectFunc();
        ...
        carbonOptions.setTabs(options.cookie);
        jQuery(document).on('click', '#options-submit', function (event) {
            event.preventDefault();
            jQuery('#carbon-options').submit()
        });
        jQuery('#carbon-content-container').delegate('.hover', 'mouseover', function () {
            jQuery(this).css({
                cursor: 'pointer'
            })
        });
        ...
        jQuery('#carbon-content-container').append('<input type="hidden" value="Upper Options Panel" />')
    },
    ...
    setHelpFunc: function () {
        jQuery('#carbon-content-container').delegate('a.help-button', 'click', function (event) {
            event.preventDefault();
            if (!carbonOptions.dialogOpened) {
                jQuery(this).find('.help-dialog:first').clone().dialog({
                    autoOpen: true,
                    title: jQuery(this).attr('title'),
                    closeText: '',
                    open: function () {
                        carbonOptions.dialogOpened = true
                    },
                    close: function () {
                        carbonOptions.dialogOpened = false
                    }
                })
            }
        })
    ...
    setOnOffFunc: function () {
		var carbon_is_rtl = jQuery('body').hasClass('rtl');
    ...
    loadUploader: function (element, pathToPhp, uploadsUrl, multi) {
        ...
        new AjaxUpload(button, {
			...
			data: {
				...
				security: jQuery('input#carbon-theme-upload-handler').val()
			},
    ...
    showSavedImgData: function (optionsData) {
        ...
        if (optionsData.hiddenIds[i]){
		    for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(carbonOptions.separator)
	        } 
	        for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(carbonOptions.separator)
	        }
	        ...
	        for (var j = 0; j < entryCount - 1; j++) {
	            ...
	            for (var i = 0; i < count; i++) {
	                if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
	                    html += carbonOptions.generatePreview(data[i][j])
	                }
    ...
    setCustomFieldsFunc: function (mainId, fieldIds, labels, istextarea, preview) {
        ...
        carbonOptions.setCommonAddFunc(optionsData)
    },
    setCommonAddFunc: function (optionsData) {
        carbonOptions.showSavedImgData(optionsData);
        jQuery(optionsData.addButton).on("click", function (event) {
            event.preventDefault();
            carbonOptions.addItem(optionsData)
        });
        jQuery(optionsData.ulId).on('sortstop', function (event, ui) {
            carbonOptions.setSliderImgChanges(optionsData)
        });
        carbonOptions.setActionButtonHandlers(optionsData)
    },
    addItem: function (optionsData) {
        ...
        for (var i = 0; i < length; i++) {
            if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
                html += carbonOptions.generatePreview(jQuery(optionsData.inputIds[i]).attr("value"))
            }
    ...
    setSliderImgChanges: function (optionsData) {
        ...
        jQuery(optionsData.ulId + ' li').each(function () {
            for (i = 0; i < count; i++) {
                values[i] += jQuery(this).find('span.' + optionsData.spanClasses[i]).html() + carbonOptions.separator
            }
        });
        for (i = 0; i < count; i++) {
...
var carbon_StyleOptionsManager = {
    separator: '|*|',
    dialogOpened: false,
    init: function (options) {
        carbon_StyleOptionsManager.setCheckboxClickHandlers();
        carbon_StyleOptionsManager.setHelpFunc();
        carbon_StyleOptionsManager.setOnOffFunc();
        carbon_StyleOptionsManager.setTextImageFunc();
        carbon_StyleOptionsManager.setLeftRightFunc();
        carbon_StyleOptionsManager.setLightDarkFunc();
        carbon_StyleOptionsManager.setColorpickFunc();
        carbon_StyleOptionsManager.setStyleSelectFunc();
        jQuery(".sortable").sortable();
        var mainNavOptions = {};
        if (options.cookie) {
            mainNavOptions = {
        ...
        jQuery(document).on('click', '#options-submit', function (event) {
            event.preventDefault();
            jQuery('#carbon-style-options').submit()
        });
        jQuery('#carbon-content-container').delegate('.hover', 'mouseover', function () {
            jQuery(this).css({
                cursor: 'pointer'
		...
		carbon_StyleOptionsManager.setTabs(options.cookie);
    },
    setTabs: function (enableCookies) {
		jQuery('.main-navigation-container').hide();
        var selectedClass = 'ui-tabs-selected',
    ...
    setHelpFunc: function () {
        jQuery('#carbon-content-container').delegate('a.help-button', 'click', function (event) {
            event.preventDefault();
            if (!carbon_StyleOptionsManager.dialogOpened) {
                jQuery(this).find('.help-dialog:first').clone().dialog({
                    autoOpen: true,
                    title: jQuery(this).attr('title'),
                    closeText: '',
                    open: function () {
                        carbon_StyleOptionsManager.dialogOpened = true
                    },
                    close: function () {
                        carbon_StyleOptionsManager.dialogOpened = false
                    }
                })
            }
        })
    ...
    showSavedImgData: function (optionsData) {
        ...
        if (optionsData.hiddenIds[i]){
		    for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(carbon_StyleOptionsManager.separator)
	        } 
	        for (var i = 0; i < count; i++) {
	            data[i] = jQuery(optionsData.hiddenIds[i]).val().split(carbon_StyleOptionsManager.separator)
	        }
	        ...
	        for (var j = 0; j < entryCount - 1; j++) {
	            ...
	            for (var i = 0; i < count; i++) {
	                if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
	                    html += carbon_StyleOptionsManager.generatePreview(data[i][j])
	                }
	                var none = data[i][j] === '' ? '<i>---</i>' : '';
	                html += '<b>' + optionsData.labels[i] + ': </b><span class="' + optionsData.spanClasses[i] + '">' + data[i][j] + '</span>' + none + '<br/>'
	            }
    ...
    setCustomFieldsFunc: function (mainId, fieldIds, labels, istextarea, preview) {
        ...
        carbon_StyleOptionsManager.setCommonAddFunc(optionsData)
    },
    setCommonAddFunc: function (optionsData) {
        carbon_StyleOptionsManager.showSavedImgData(optionsData);
        jQuery(optionsData.addButton).on("click", function (event) {
            event.preventDefault();
            carbon_StyleOptionsManager.addItem(optionsData)
        });
        jQuery(optionsData.ulId).on('sortstop', function (event, ui) {
            carbon_StyleOptionsManager.setSliderImgChanges(optionsData)
        });
        carbon_StyleOptionsManager.setActionButtonHandlers(optionsData)
    },
    addItem: function (optionsData) {
        var length = optionsData.inputIds.length;
        var html = '<li>';
        for (var i = 0; i < length; i++) {
            if (optionsData.preview && optionsData.inputIds[i] === '#' + optionsData.preview) {
                html += carbon_StyleOptionsManager.generatePreview(jQuery(optionsData.inputIds[i]).attr("value"))
            }
            html += '<b>' + optionsData.labels[i] + ': </b><span class="' + optionsData.spanClasses[i] + '">' + jQuery(optionsData.inputIds[i]).val() + '</span><br/>'
        }
        html += '<div class="editButton hover"></div><div class="deleteButton hover"></li>';
    ...
    setSliderImgChanges: function (optionsData) {
        ...
        jQuery(optionsData.ulId + ' li').each(function () {
            for (i = 0; i < count; i++) {
                values[i] += jQuery(this).find('span.' + optionsData.spanClasses[i]).html() + carbon_StyleOptionsManager.separator
            }
        });
        for (i = 0; i < count; i++) {
            jQuery(optionsData.hiddenIds[i]).val(values[i])
...
jQuery(window).on("load", function () {
    if (jQuery('#saved_box').length) {
        setTimeout('carbonOptions.removeSavedMessage()', 3000);
		setTimeout('carbon_StyleOptionsManager.removeSavedMessage()', 3000);
    }
});
...
jQuery(document).on("click", "#carbon_demos_container .theme-browser .theme-actions a", function(){
	...
	if (confirmdemo == true){
		...
		if (jQuery(".carbon_demo_status").data("uiDialog")) jQuery(".carbon_demo_status").dialog("destroy");
		jQuery(".carbon_demo_status").attr("title","Applying the demo").html("<span class=\'spinner is-active\'></span>Installing the theme.<br/>Status:<ul class=\'carbon_demo_progress\'></ul>").dialog({
			modal: true,
			closeOnEscape: false,
			autoOpen: false,
			draggable: false,
		...
		jQuery(".carbon_demo_status").dialog("open");
		jQuery(".carbon_demo_status").data("uiDialog").uiButtonSet.find("button").on("click", function(){
		...
		jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 0%");
		...
		jQuery.retryAjax({
			...
			data: { 
				...
				security: jQuery("input#carbon-theme-options").val()
			},
			success: function(response){
				
				jQuery(".carbon_demo_progress").append("<li>Theme Reactivation [OK]</li>");
				jQuery(".carbon_demo_status").dialog("option", "position", "center");
				
				jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 15%");
				jQuery(".carbon_demo_progress").append("<li class=\'des_step_plugins\'>Installing Plugins...</li>");
				...
				jQuery.retryAjax({
					...
					success: function(response){
						jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 35%");
						jQuery(".carbon_demo_progress .des_step_plugins").html("Plugins Installed [OK]");
						jQuery(".carbon_demo_status").dialog("option", "position", "center");

						// set panel options
						jQuery(".carbon_demo_progress").append("<li class=\'des_step_panels\'>Setting Panels Options...</li>");
						var xmlPath = "https://demos.upperthemes.com/demos/carbon/"+lo_demo+"/options.xml";
						var xmlStylePath = "https://demos.upperthemes.com/demos/carbon/"+lo_demo+"/style_options.xml";
						...
						jQuery.retryAjax({
							...
							success: function (response) {
								jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 60%");
								jQuery(".carbon_demo_progress .des_step_panels").html("Panels Options [OK]");
								jQuery(".carbon_demo_status").dialog("option", "position", "center");
								jQuery(".carbon_demo_progress").append("<li class=\'des_step_contents\'>Importing Contents...</li>");
								var desitimeout = Math.floor(Math.random() * 12) + 3;
								var incre = Math.floor(Math.random() * 2) + 1;
								var perc = 60;
								carbon_import_percentage(desitimeout,incre,perc);
								...
								jQuery.ajax({
									...
									success: function(response){
										...
										jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 90%");
										jQuery(".carbon_demo_progress .des_step_contents").html("Import Contents [OK]");
										jQuery(".carbon_demo_progress").append("<li>Set Menu [OK]</li>");
										jQuery(".carbon_demo_status").dialog("option", "position", "center");
										jQuery(".carbon_demo_progress").append("<li class=\'des_step_widgets\'>Importing Widgets...</li>");
										...
										jQuery.retryAjax({
											...
											success: function(response){
												jQuery(".carbon_demo_status .spinner").removeClass("is-active");
												jQuery(".carbon_demo_progress .des_step_widgets").html("Import Widgets [OK]");
												jQuery(".carbon_demo_status").dialog("option", "position", "center");
												// Reload to complete. 
												jQuery(".carbon_demo_status").append("<p style=\'left:20px; line-height: 15px;\'>Process almost complete.<br/>Click OK to Continue.</p>");
												jQuery("button.ui-button.ui-state-disabled").removeClass("ui-state-disabled");
												jQuery(".carbon_demo_status").dialog("option", "title", "Applying the demo - 100%");
...
function carbon_ajax_error_handler(){ jQuery(".carbon_demo_progress").after("<div class=\'error\'>An unexpected error has occurred. Please <a href=\'#\' onclick=\'javascript:window.location=window.location;\'>refresh</a> the page and try again. If the problem persists, please <a href=\'https://upperthemes.com/support/\'>contact us</a>.</div>"); }
function carbon_import_percentage(t,o,i){window.desigtimeout=setTimeout(function(){i<90?(i+o>89&&(o=89-i),i+=o,jQuery(".carbon_demo_status").dialog("option","title","Applying the demo - "+i+"%"),t=Math.floor(12*Math.random())+5,o=Math.floor(2*Math.random())+1,carbon_import_percentage(t,o,i)):(clearTimeout(window.desigtimeout),t=!1)},1e3*t)}
...
(function(jQuery){jQuery.retryAjax=function(ajaxParams){var errorCallback;ajaxParams.tryCount=(!ajaxParams.tryCount)?0:ajaxParams.tryCount;ajaxParams.retryLimit=(!ajaxParams.retryLimit)?2:ajaxParams.retryLimit;ajaxParams.suppressErrors=true;if(ajaxParams.error){errorCallback=ajaxParams.error;delete ajaxParams.error;carbon_ajax_error_handler()}else{errorCallback=function(){console.warn('ERROR CALLBACK. Please Refresh');};}
