...
if ( isset($_GET['page']) && $_GET['page'] == CARBON_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'carbon_init_options_functionality');  
	add_action('init', 'carbon_set_options'); 
}

if ( isset($_GET['page']) && $_GET['page'] == CARBON_STYLE_OPTIONS_PAGE ){
	//options actions
	add_action('init', 'carbon_init_style_options_functionality');  
	add_action('init', 'carbon_set_style_options'); 
}
 ...
 */
function carbon_init_style_options_functionality(){
	global $carbon_style_options;

	$carbon_style_options=array();

	//get all the categories
	$categories=get_categories('hide_empty=0');
	$carbon_categories=array();
	for($i=0; $i<sizeof($categories); $i++){
		$carbon_categories[]=array('id'=>$categories[$i]->cat_ID, 'name'=>$categories[$i]->cat_name);
	}

	//load the files that contain the options
	$carbon_style_options_files=array('style-general', 'style-website-loading', 'style-header-top-bar', 'style-logotype', 'style-menu', 'style-sliding-panel', 'style-search', 'style-pagetitle', 'style-footer', 'style-typography' , 'style-import-export');
	foreach($carbon_style_options_files as $file){
		require_once(CARBON_OPTIONS_PATH.$file.'.php');
	}
}
...
function carbon_set_style_options(){
	...
	if ( $_GET['page'] == CARBON_STYLE_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['carbon-theme-style-options'],'carbon-theme-update-style-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("carbon_style_first_save")){
						update_option("carbon_style_first_save",'true');
					}
					...
					$doc->save($filename);		
					header("Location: themes.php?page=".CARBON_STYLE_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 * to populate the global options array.
 */
function carbon_init_options_functionality(){
	global $carbon_options;

	$carbon_options=array();
	...
	//load the files that contain the options
	$carbon_options_files=array('general', 'header-menu', 'widgetsarea', 'sliders', 'translation', 'social', 'data');
	if (defined('CARBON_PLG_ACTIVE') === true) $carbon_options_files = array('general', 'header-menu', 'widgetsarea', 'sliders', 'newsletter', 'translation', 'social', 'customcss' , 'data');
	
	foreach($carbon_options_files as $file){
		require_once(CARBON_OPTIONS_PATH.$file.'.php');
	}
}
...
function carbon_set_options(){
	...
	//insert the default values if the fields are empty
	foreach ($carbon_options as $value) {
		if(isset($value['id'])){
			if(get_option($value['id'])=='' && isset($value['std']) && !in_array($value['type'], $nonsavable_types)){
				update_option( $value['id'], $value['std']);
			}
	...
	//save the field's values if the Save action is present
	if ( $_GET['page'] == CARBON_OPTIONS_PAGE ) {
	
		if (isset($_REQUEST['action'])){
			if ( 'save' == $_REQUEST['action'] ) {
				//verify the nonce
				if ( empty($_POST) || !wp_verify_nonce($_POST['carbon-theme-options'],'carbon-theme-update-options') ){
					print 'Sorry, your nonce did not verify.';
					exit;
				} else {
					
					if (!get_option("carbon_first_save")){
						update_option("carbon_first_save",'true');
					}
					...
					foreach ($carbon_options as $value) {
						...
						if (isset($value['id'])){
							if( isset( $_REQUEST[ $value['id'] ] ) && !in_array($value['type'], $nonsavable_types)) {
								...
								if ($value['id'] === "carbon_portfolio_permalink"){
									$oldval = get_option("carbon_portfolio_permalink");
					...
					$doc->appendChild($root);
					$doc->save($filename);		
					header("Location: themes.php?page=".CARBON_OPTIONS_PAGE."&saved=true");
					die;
				}
			}	
		}
 ...
 */
function carbon_theme_admin() {
	global $carbon_options,$carbon_options_manager;

	$carbon_options_manager=new CarbonOptionsManager("Carbon", CARBON_IMAGES_URL, CARBON_UTILS_URL, CARBON_UPLOADS_URL, CARBON_VERSION);
	$carbon_options_manager->set_options($carbon_options);

	if ( isset($_REQUEST['saved'] )) {
		$carbon_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$carbon_options_manager->print_reset_message();
	}

	$carbon_options_manager->print_heading("");
	$carbon_options_manager->print_options();
	$carbon_options_manager->print_footer();
}
 ...
 */
function carbon_theme_style_options_admin() {
	global $carbon_style_options, $carbon_style_options_manager;

	$carbon_style_options_manager=new CarbonStyleOptionsManager("Carbon", CARBON_IMAGES_URL, CARBON_UTILS_URL, CARBON_UPLOADS_URL, CARBON_VERSION);
	$carbon_style_options_manager->set_options($carbon_style_options);

	if ( isset($_REQUEST['saved'] )) {
		$carbon_style_options_manager->print_saved_message();
	}
	if ( isset($_REQUEST['reset'] )) {
		$carbon_style_options_manager->print_reset_message();
	}

	$carbon_style_options_manager->print_heading("");
	$carbon_style_options_manager->print_options();
	$carbon_style_options_manager->print_footer();
}
 ...
 */
function carbon_theme_demos_admin() {
	global $carbon_demos_manager;

	$carbon_demos_manager=new CarbonDemosManager("Carbon", CARBON_IMAGES_URL, CARBON_UTILS_URL, CARBON_UPLOADS_URL, CARBON_VERSION);

	$carbon_demos_manager->print_heading("");
	$carbon_demos_manager->print_options();
	$carbon_demos_manager->print_footer();
}
 ...
 */
function carbon_add_options($option_arr){
	global $carbon_options;

	foreach($option_arr as $option){
		$carbon_options[]=$option;
	}
}

function carbon_add_style_options($option_arr){
	global $carbon_style_options;

	foreach($option_arr as $option){
		$carbon_style_options[]=$option;
	}
}
 ...
 */
function carbon_get_opt($option){
	return stripslashes(get_option("carbon".$option));
}


function carbon_get_custom_sidebars(){
	$res=array();
	$sides=get_option('carbon_sidebar_name_names');
	if($sides){
		$res=explode(CARBON_SEPARATOR, $sides);
		array_pop($res);
	}
	return $res;
}
 ...
 */
function carbon_add_custom_options($opt_array){
	$new_carbon_options=array();
	...
	foreach($opt_array as $option){
		if($option['type']=='multiple_custom'){
			...
			$saved_array=explode(CARBON_SEPARATOR, $saved_values);
			if(sizeof($saved_array)>1){
				array_pop($saved_array);
				foreach($saved_array as $custom_name){
					$id=carbon_convert_to_class($custom_name);
					$custom_option=array(
					"id"=>$id,
					...
					array_push($new_carbon_options, $custom_option);
				}
			}
				
		} else{
			//this is just a normal option, just append it into the new array
			array_push($new_carbon_options, $option);
		}
	}

	return $new_carbon_options;
}

function carbon_fonts_array_builder(){
	$defaultfonts = array();
...
function carbon_portfolio_types(){
...
function carbon_get_proj(){
	
	$carbon_proj2=array(array('id'=>'default', 'name'=>'Default'));
	if (function_exists('carbon_get_projects')){
		$carbon_proj1 = carbon_get_projects();
		foreach($carbon_proj1 as $dp){
			$carbon_proj2[]=array("name"=>$dp['p_title'], "id"=>$dp['p_id']);
		}
	}
...
function carbon_get_all_google_fonts(){
	$carbon_google_fonts = array(	array( 'name' => "Cantarell", 'variant' => ':r,b,i,bi'),
