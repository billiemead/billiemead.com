...
function upper_demo_installer(){ 
	...
	if (!wp_verify_nonce($_POST['security'],'carbon-theme-update-options')) return;
	...
	$demosurl = "https://demos.upperthemes.com/demos/carbon/";
	$carbon_stylesheet = get_option('stylesheet') ? get_option('stylesheet') : "carbon";
	...
	switch($_POST['upper_action']){
		...
		case 'dbreset': 
			...
			try {
				...
				$carbonsiteurl = get_option('siteurl');
				$carbonhome = home_url('/');
				...
				if ( $db_tables = $wpdb->get_col("SHOW TABLES LIKE '{$wpdb->prefix}%'") ) {
					...
					$keys['url'] = $carbonsiteurl;
											
					carbon_wp_update_user($user, $keys);
				}

				switch_theme( $carbon_stylesheet );
				
				update_option('siteurl',$carbonsiteurl);
				update_option('home',$carbonhome);
		...
		case 'install_plugins':
			try{
				$plugins_url = "https://demos.upperthemes.com/plugins/carbon/";
				$plugins = array(
					array('name' => 'carbon_custom_post_types', 'path' => $plugins_url . 'carbon_custom_post_types.zip', 'install' => 'carbon_custom_post_types/carbon_custom_post_types.php'),
				...
				$plgs = carbon_get_plugins($plugins);
			} catch (Exception $e){
				$errors = $e->getMessage();
			}
			flush();
		...
		case 'import_content_set_options':
			...
			try{
				...
				// import content
				carbon_autoimport($demosurl, $demo);
				...
				update_option("theme_mods_".$carbon_stylesheet, $mods);
				update_option("mods_carbon", $mods);
		...
		case 'import_widgets':
			...
			ob_start();
			carbon_wie_process_import_file($demosurl . $demo . "/" . $filename);
			ob_end_clean();
			if ( !defined('RS_PLUGIN_PATH') ) carbon_plugin_activate('revslider/revslider.php');
			flush();
			ob_flush();
			sleep(1);
			wp_send_json_success( $errors, 200 );
		...
		case 'complete-installation':
			...
			//rev
			try{
				if (function_exists('carbon_add_increase_time')) carbon_add_increase_time();
				
				$dir = "https://demos.upperthemes.com/demos/carbon/" . $_POST['upper_demo'] . "/revdemos/";
				...
				if ($thefile != ""){
					...
					foreach($zips as $zip){
						if ($zip != ""){
							...
							if (!carbon_add_imported_id( $slug, true )) continue;
				...
				$cubefp = "https://demos.upperthemes.com/demos/carbon/" . $_POST['upper_demo'] . "/cubeportfolio.json";
				...
				//MASTERS
				$mastersfp = "http://demos.upperthemes.com/demos/carbon/" . $_POST['upper_demo'] . "/masterslider.json";
...
function carbon_wie_process_import_file( $file ) {
	WP_Filesystem();
	...
	if ($data){
		ob_start();
		carbon_wie_import_data( $data );
		ob_end_clean();		
	}
}

function carbon_wie_available_widgets() {
...
function carbon_wie_import_data( $data ) {

	global $wp_registered_sidebars;
	$available_widgets = carbon_wie_available_widgets();
	...
	foreach ( $data as $sidebar_id => $widgets ) {
		...
		foreach ( $widgets as $widget_instance_id => $widget ) {
			...
			if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
				...
				$widget_message = esc_html__( 'Site does not support widget', 'carbon' ); // explain why widget not imported
			}
			...
			if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
				...
				foreach ( $single_widget_instances as $check_id => $check_widget ) {
					...
					if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
						...
						$widget_message = esc_html__( 'Widget already exists', 'carbon' ); // explain why widget not imported
			...
			if ( ! $fail ) {
				...
				if ( $sidebar_available ) {
					...
					$widget_message = esc_html__( 'Imported', 'carbon' );
				} else {
					$widget_message_type = 'warning';
					$widget_message = esc_html__( 'Imported to Inactive', 'carbon' );
				}
			...
			$results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : esc_html__( 'No Title', 'carbon' ); // show "No Title" if widget instance is untitled
 ...
 */
function carbon_wp_update_user($user, $keys) {
	global $wpdb;			
...
function carbon_get_plugins($plugins){
	$args = array(
	...
	foreach($plugins as $plugin){
		...
		//if (!file_exists(ABSPATH.'wp-content/plugins/'.$plugin['install'])){
			carbon_plugin_download($plugin['path'], $args['path'].$plugin['name'].'.zip');
			carbon_plugin_unpack($args, $args['path'].$plugin['name'].'.zip');
		//}
		/* leave revslider for later. */
		if ($plugin['name'] != "revslider") {
			$output[] = $plugin;
			carbon_plugin_activate($plugin['install']);
		}
	}
	return $output;
}

function carbon_plugin_download($url, $path){
...
function carbon_plugin_unpack($args, $target){
	global $wp_filesystem;
...
function carbon_plugin_activate($installer){
