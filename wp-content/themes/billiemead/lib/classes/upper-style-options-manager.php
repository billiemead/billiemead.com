 ...
 */
class CarbonStyleOptionsManager{
	...
	/**
	 * The main constructor for the CarbonStyleOptionsManager class
	...
	function print_heading($heading_text){
		...
		echo '<div id="carbon-content-container"><form method="post" id="carbon-style-options">';
		if ( function_exists('wp_nonce_field') ){
			wp_nonce_field('carbon-theme-update-style-options','carbon-theme-style-options');
	...
	function print_footer(){
		echo '</div></div><div id="carbon-footer"><div id="follow-upper"> 
			 <p>Follow us</p><ul>
			 <li><a href="http://facebook.com/UpperInc" title="Upper on Facebook"><img src="'.esc_url($this->carbon_images_url).'facebook.png" /></a></li>
			 <li><a href="http://twitter.com/UpperInc" title="Upper on Twitter"><img src="'.esc_url($this->carbon_images_url).'twitter.png" /></a></li>
	...
	function print_upload($value){
		...
		$carbon_admin_inline_script .= '
			jQuery(document).ready(function(){
				"use strict";
				carbon_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($value['id']).'_button"), "'.esc_js($this->carbon_utils_url).'upload-handler.php", "'.esc_js($this->carbon_uploads_url).'");
			});
		';
	...
	function print_stylebox($value, $type){
		...
		foreach ($value['options'] as $option) {
			...
			echo '<li onclick="jQuery(this).parents(\'.sub-navigation-container\').find(\'#'."carbon".'_style_color\').attr(\'value\',\''.esc_js($option).'\'); jQuery(this).parents(\'.sub-navigation-container\').find(\'.color-preview\').css(\'background-color\',\'#'.esc_js($option).'\');" style'.'="'.esc_attr($style).'" class="'.esc_attr($class).'"><a class="style-box" title="'.esc_attr($option).'" href=""></a></li>';
		} 
		echo '</ul></div>';
		$this->close_option($value);
	}
	...
	function print_custom($value){
			...
			switch($field['type']){
				...
				case 'upload':
					...
					$carbon_admin_inline_script .= '
						jQuery(document).ready(function(){
							"use strict";
							carbon_StyleOptionsManager.loadUploader(jQuery("div#'.esc_js($field['id']).'_button"), "'.esc_js($this->carbon_utils_url).'upload-handler.php", "'.esc_js($this->carbon_uploads_url).'");
						});
					';
					wp_add_inline_script('carbon-admin', $carbon_admin_inline_script, 'after');
		...
		$updir = wp_upload_dir();
		$optsxml = file_exists($updir['basedir']."/style_options.xml") ? $updir['baseurl']."/style_options.xml" : get_template_directory_uri() . "/carbon_original_panel_style_options.xml";
		...
		$carbon_admin_inline_script .= '
			jQuery(document).ready(function(){
				...
				jQuery(\'#carbon_export_style_options_button\').css({\'position\':\'relative\',\'float\':\'left\'}).attr(\'target\',\'_blank\').unbind().off("click").on("click",function(){
					window.open("'.$optsxml.'");
				});
			});
		';
