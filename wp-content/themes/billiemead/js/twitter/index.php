...
if (!class_exists("ezTweet")){
	class ezTweet {
		...
		public function __construct() {
			...
			$this->lib = $_POST['templatepath'].'wp-content/plugins/carbon_custom_post_types/lib/';
		...
		private function getTwitterJSON() {
			...
			if (!isset($_POST['request'])){
				$_POST['request'] = array(
					...
					"parameters" => array(
						"include_entities" => 1,
						"screen_name" => array(get_option('carbon_twitter_username')),
						"page" => 1,
						"count" => intval(get_option('carbon_twitter_number_tweets')),
						"include_rts" => 1
					)
				);
			}
...
function upper_ajax_twitter(){
	...
	if (!wp_verify_nonce($_POST['security'],'carbon-theme-twitter')) return;
	
	include_once $_POST['templatepath'] . 'wp-content/plugins/carbon_custom_post_types/lib/tmhOAuth.php';
