 ...
 */
	...
	$carbon_more = 0; 
		...
		if (is_front_page() && function_exists('vc_is_inline') && !vc_is_inline()){
					...
					if ($template === "template-home.php" && $firstHome){
						...
						switch($homeType){
							case "slider":
								...
								carbon_print_slider($thisID, $prlx);
							break;
									...
									<?php
										if ($homeType == "image"){
											...
											<div id="parallax-home" <?php if (get_post_meta($thisID, 'parallaxEffect_value', true) == 'yes') echo 'class="parallax" data-stellar-ratio="0.5" '; ?> >
												<?php carbon_print_intro($thisID); ?>
											</div>
											<?php
											$carbon_output = "#parallax-home{ background-image: url(".esc_url($media[1]).");background-size:cover;text-align:center; }";
											carbon_set_custom_inline_css($carbon_output);
										} else {
											?>
											<div id="parallax-home" <?php if (get_post_meta($thisID, 'parallaxEffect_value', true) == 'yes') echo 'class="parallax" data-stellar-ratio="0.5"'; ?>>
												<?php carbon_print_intro($thisID, true); ?>
						...
						<section class="page_content" id="section_page-<?php echo esc_attr($thisID); ?>">
							...
							<?php echo apply_filters('the_content', get_post_field('post_content', $thisID)); 
								...
								if ( ! empty( $post_custom_css ) ) {
									...
									carbon_set_custom_inline_css($post_custom_css);
								}
							?>
							</div>
						</section>
			...
			carbon_print_menu(true); $carbon_color_code = get_option("carbon_style_color");
			
			if (get_post_meta($this_page_id, "carbon_enable_custom_pagetitle_options_value", true) == "no" || !get_post_meta($this_page_id, "carbon_enable_custom_pagetitle_options_value", true)){
				...
				$type = get_post_meta($this_page_id, "carbon_header_type_value", true);
				$thecolor = carbon_hex2rgb(get_post_meta($this_page_id, "carbon_header_color_value", true)); 
				$opacity = intval(str_replace("%","",get_post_meta($this_page_id, "carbon_header_color_opacity_value", true)))/100;
				$color = "rgba(".$thecolor[0].",".$thecolor[1].",".$thecolor[2].",".$opacity.")";
				$image = get_post_meta($this_page_id, "carbon_header_image_value", true);
				$image = explode('|!|',$image);
				if (isset($image[1])) $image = explode('|*|',$image[1]);
				$image = $image[0];
				$pattern = CARBON_PATTERNS_URL.get_post_meta($this_page_id, "carbon_header_pattern_value", true).".jpg";
				$custompattern = get_option("carbon_header_custom_pattern_value"); 
				$margintop = get_post_meta($this_page_id, "carbon_header_text_margin_top_value", true);
				$banner = get_post_meta($this_page_id, "carbon_banner_slider_value", true);
				$showtitle = get_post_meta($this_page_id, "carbon_hide_pagetitle_value", true) == "yes" ? true : false;
				$showsectitle = get_post_meta($this_page_id, "carbon_hide_sec_pagetitle_value", true) == "yes" ? true : false;
				$tcolor = get_post_meta($this_page_id, "carbon_header_text_color_value", true);
				$tsize = intval(str_replace(" ", "", get_post_meta($this_page_id, "carbon_header_text_size_value", true)),10)."px";
				$tfont = get_post_meta($this_page_id, "carbon_header_text_font_value", true);
				$stcolor = get_post_meta($this_page_id, "carbon_secondary_title_text_color_value", true);
				$stsize = intval(str_replace(" ", "", get_post_meta($this_page_id, "carbon_secondary_title_text_size_value", true)),10)."px";
				$stfont = get_post_meta($this_page_id, "carbon_secondary_title_font_value", true);
				$stmargin = intval(str_replace(" ", "", get_post_meta($this_page_id, "carbon_header_secondary_text_margin_top_value", true)),10)."px";
				$originalalign = get_post_meta($this_page_id, "carbon_header_text_alignment_value", true);
				$pt_parallax = get_post_meta($this_page_id, "carbon_pagetitle_image_parallax_value", true) == "on" ? true : false;
				$pt_overlay = get_post_meta($this_page_id, "carbon_pagetitle_image_overlay_value", true) == "on" ? true : false;
				$pt_overlay_type = get_post_meta($this_page_id, "carbon_pagetitle_overlay_type_value", true);
				$pt_overlay_the_color = carbon_hex2rgb(get_post_meta($this_page_id, "carbon_pagetitle_overlay_color_value", true));
				$pt_overlay_pattern = CARBON_PATTERNS_URL.get_post_meta($this_page_id, "carbon_pagetitle_overlay_pattern_value", true);
				$pt_overlay_opacity = intval(str_replace("%","",get_post_meta($this_page_id, "carbon_pagetitle_overlay_opacity_value", true)))/100;
				$pt_overlay_color = "rgba(".$pt_overlay_the_color[0].",".$pt_overlay_the_color[1].",".$pt_overlay_the_color[2].",".$pt_overlay_opacity.")";
				$breadcrumbs = get_post_meta($this_page_id, "carbon_enable_breadcrumbs_value", true) == "yes" ? "on" : "off";
				$breadcrumbs_margintop = intval(str_replace(" ", "", get_post_meta($this_page_id, "carbon_breadcrumbs_margin_top_value", true)),10)."px";
				$pagetitlepadding = intval(str_replace(" ", "", get_post_meta($this_page_id, "carbon_page_title_padding_value", true)),10)."px";
			}
			$height = "auto";
						...
						<div class="container present-container <?php echo esc_attr($originalalign); ?>">
							...
							<?php
								...
								if ($showtitle){
									?>
									<h1 class="page_title">
										<?php if (is_front_page() && is_home()) echo wp_kses_post(get_option('carbon_blog_single_primary_title')); else echo wp_kses_post(get_the_title($this_page_id)); ?>
									</h1>
									<?php
